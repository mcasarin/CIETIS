...
if(isset($_REQUEST) && count($_REQUEST) > 0){
    ...
    $return = checkInscricao($check);
    

    if($return == '0'){
        $msg = "<label class='text-success text-center'>Sua inscrição foi ativada com sucesso!</label>
        <br /><label class='text-warning'>Porém, é necessário completa-la com o pagamento da taxa e envio do comprovante.</label>";
        $btn = "<input class='btn btn-outline-info' type='submit' name='envioArquivo' value='Enviar'>";
    } elseif($return == '1') {
        $msg = "<label class='text-warning text-center'>Inscrição já foi ativada.</label>
        <br /><label class='text-danger'>Pendente o envio de comprovante de pagamento da inscrição.</label>";
        $btn = "<input class='btn btn-outline-info' type='submit' name='envioArquivo' value='Enviar'>";
    } elseif($return == '2') {
        $msg = "<p class='text-success text-center'>Inscrição já foi ativada!
        <br />Comprovante de pagamento já foi enviado!<br /><br />
        Agora é só aguardar o contato da coordenação do evento com envio das salas e links!</p>";
        $btn = "<span class='alert alert-success text-center'>Comprovante já foi enviado.</span>";
    } else {
